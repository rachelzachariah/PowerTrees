...
def pv_eqs(G):
	...
	S = PolynomialRing(QQ,2*(n-1)+len(b_names),names=x_names+y_names+b_names)
	v = S.variable_names()
	B = Matrix(S,n,n)
	b = v[2*(n-1):]
	t = 0
...
parser.add_argument('-s', nargs=6, type=float, dest="start_pt", default = [-50,-100,-100,-100,-100,-100])
...
p = S((S.ideal(g_eqs+h_eqs)).elimination_ideal(elim_vars).gens()[0]/xy_vars_indet[3])
c = p.coefficients()
s = 0
degr = len(c)-1
